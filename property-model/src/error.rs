...
pub enum Error {
    ...
    /// relationship was not found, a relationship that involves the match cell has more
    /// than one method, a relationship appears in more than one conditional branch,
    /// a branch key's type does not match the cell's registered type, or a branch has
    /// no keys.
    InvalidConditional,
}
