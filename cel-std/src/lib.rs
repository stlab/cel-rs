//! CEL standard library: `round`, `min`, `max`, `clamp`, and related numeric functions built on
...
/// Registers every CEL standard-library function on `lookup`: `round`, `min`, `max`,
/// `clamp`, `abs`, `signum`, `sqrt`, `floor`, `ceil`, and `trunc`.
///
...
pub fn install(lookup: &mut cel_parser::OpLookup) {
    lookup.push_library_scope(math::round_scope);
