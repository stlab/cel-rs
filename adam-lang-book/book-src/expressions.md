...
evaluated by `cel-parser`/`cel-runtime`, not by adam-lang. Literals, arithmetic and comparison
...
covers only what adam-lang does *around* an expression: deciding which cells it may read, and
what it does with the value it produces.

## 3.2 No standard library of its own

adam-lang defines no functions. `min`, `max`, `clamp`, `round`, and anything else callable from
...
install `cel-std`, exactly as `begin` does (see `support::parser` in `adam-lang-book`'s own
source). A parser built with a bare `OpLookup::new()` and no library installed can still parse
...
than once (`a && a`) still counts as one input, not two. This is why adam-lang has no forward
