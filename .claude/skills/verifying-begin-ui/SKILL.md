...
2. **Screenshot** (visual check):
   ...
   "/c/Program Files (x86)/Microsoft/Edge/Application/msedge.exe" \
     ...
     --virtual-time-budget=8000 --screenshot=/tmp/shot.png http://localhost:8080
   ```
...
3. **DOM dump** (structural check - catches corrupted tag names, missing
   ...
   ```bash
   msedge.exe --headless=new --disable-gpu --no-sandbox --virtual-time-budget=8000 \
     --dump-dom http://localhost:8080 > /tmp/dom.html
   ```
...
4. **Live JS/CSS introspection** (when the DOM and screenshot alone don't
   ...
   ```bash
   msedge.exe --headless=new --disable-gpu --no-sandbox --remote-debugging-port=9222 http://localhost:8080 &
...
- Always `taskkill //F //IM msedge.exe` and `//IM dx.exe` when done - orphaned
  headless instances hold the port/profile and cause the next run to hang or
  connect to stale state.
- Use a fully detached background launch (`nohup ... & disown`, or the
  harness's own background-task support) - a bare trailing `&` inside a
  foreground call can die when that call returns.
